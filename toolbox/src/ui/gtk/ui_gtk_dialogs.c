...
void on_explorer_menu_properties(GtkMenuItem *item, gpointer user_data) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(store), &ctx->iter, EXPLORER_COL_NAME, &name, EXPLORER_COL_PATH,
                        &relative_path, EXPLORER_COL_IS_DIR, &is_dir, EXPLORER_COL_NODE_ID, &node_id, -1);
    ...
    add_form_row(GTK_GRID(grid), row++, "Read-only", gtk_label_new(node && node->read_only ? "Yes" : "No"));
    add_form_row(GTK_GRID(grid), row++, "Executable", gtk_label_new(node && node->executable ? "Yes" : "No"));
