...
typedef struct ToolPanelManifest {
    char title[128];
    char data_file_absolute_path[4096]; /* resolved + containment-checked against tools_root */
 ...
 * known by the caller to resolve inside tools_root (e.g. via
 ...
 * it via workspace_root_resolve_path() against tools_root - the same
 * sandboxing discipline every other tools/files path in this app goes
