 ...
 * explorer sidebar shows two permanent top-level rows (Files, backed
 * by FileTree/files/; Tools, backed by tools_index/tools/), each
 ...
 * resolution finds the *same* physical files/tools directories for
...
static gboolean drive(gpointer user_data) {
    ...
    /* Refresh must not duplicate Files's top-level rows. */
    ...
    if (gtk_tree_model_iter_n_children(model, &workbench_iter) != 2) {
        fail(test, "expected Files to still show exactly 2 entries after refresh (no duplicates)");
        goto done;
    }

    /* The two sources never cross-contaminate. */
    if (gtk_tree_model_iter_n_children(model, &tools_iter) != 1) {
        fail(test, "expected exactly 1 entry under Tools");
        goto done;
    }
    GtkTreeIter tool_iter;
    if (!find_child_by_name(model, &tools_iter, "tool_a.sh", &tool_iter)) {
        fail(test, "expected 'tool_a.sh' under Tools");
        goto done;
    }
    if (find_child_by_name(model, &workbench_iter, "tool_a.sh", &tool_iter)) {
        fail(test, "Tools's fixture leaked into Files");
        goto done;
    }
    if (find_child_by_name(model, &tools_iter, "afolder", &afolder_iter) ||
        find_child_by_name(model, &tools_iter, "bfile.txt", &bfile_iter)) {
        fail(test, "Files's fixtures leaked into Tools");
        goto done;
    }
...
int main(void) {
    ...
    snprintf(tool_file, sizeof(tool_file), "%s/tool_a.sh", tools_index_dir());
    ...
    g_print("explorer_sidebar_smoke: merged Files/Tools roots, eager root load, lazy subfolder expand, "
